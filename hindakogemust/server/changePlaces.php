...
if (isset($_POST["del"])) {
	$sql = "DELETE FROM 155376_hindaplaces WHERE ID='" . $_POST["del"] . "';";
...
echo "<div id='choose'>Administreerimine</div>";
echo "<div id='tableadmin'><form method='post'><input type='text' name='search' id='search' value='" . $_SESSION["search"] . "' placeholder='Otsi'></form>";
...
echo "<th><form method='post' id='colrated'><input type='submit' value='Hinnatud (korda)' id='cellbutton' name='rated'></form></th>";
echo "<th id='coladmin'>Admin</th></tr>";

$x = 0;
while ($row = mysqli_fetch_array($result)) {
	if ($x == 10) { $x = 11; break; }
	echo "<tr><th><a href='new/?index=" . $row["ID"] . "' id='invis'>Edit</a></th>";
	echo "<td>" . $row["name"] . "</td>";
	echo "<td>" . $row["address"] . "</td>";
	echo "<td>" . round($row["average"], 1) . "</td>";
	echo "<td>" . $row["rated"] . "</td>";
	echo "<th>" . "<form method='post'><input type='submit' value='Kustuta' id='delbut'><input name='del' value='".$row["ID"]."' style='display: none;'></form>" . "</th></tr>";
	$x += 1;
}
echo "</table>";
...
echo "</form><p>";
echo "<a href='new/' id='new'>Lisa uus</a></div>";
?>